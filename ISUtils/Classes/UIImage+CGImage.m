...
+ (void)load {
    SwizzleClassMethod(self, @selector(imageNamed:), @selector(swizzle_imageNamed:));
}
...
+ (UIImage *)swizzle_imageNamed:(NSString *)name {
    ...
    return [self swizzle_imageNamed:name];
}
...
+ (UIImage *)QRImageWithString:(NSString *)text qrSize:(CGSize)size qrColor:(UIColor *)qrColor bkColor:(UIColor *)bkColor {
    ...
    if (qrColor) {
        color1 = [CIColor colorWithCGColor:qrColor.CGColor];
    }
    if (bkColor) {
        color2 = [CIColor colorWithCGColor:bkColor.CGColor];
    }
    //上色
